...
class PersistedState {
    ...
    initIpcConnectionToMain() {
        ...
        electron_better_ipc_1.ipcRenderer.on(helpers_1.ipcEvents.COMMIT, (_event, { type, payload }) => {
            this.store.commit(type, payload);
        });
        electron_better_ipc_1.ipcRenderer.on(helpers_1.ipcEvents.DISPATCH, (_event, { type, payload }) => {
    ...
    */
    static getStoreFromRenderer() {
        ...
        const commit = (type, payload) => {
            ...
            connection.send(helpers_1.ipcEvents.COMMIT, { type, payload });
        };
        const dispatch = (type, payload) => {
            if (!connection)
                throw new Error('[Vuex Electron] Not connected to renderer.');
            connection.send(helpers_1.ipcEvents.DISPATCH, { type, payload });
        };
        const getState = () => {
            if (!connection)
                throw new Error('[Vuex Electron] Not connected to renderer.');
